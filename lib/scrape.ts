...
function findJobPostingLdJson(html: string): LdJobPosting | null {
  const scripts = [
    ...html.matchAll(
...
export async function scrapeJobPage(url: string): Promise<ScrapeResult> {
  ...
  const description =
    ld?.description || (ogDescription ? ogDescription.trim() : '') || (metaDescription ? metaDescription.trim() : '')
