...
export default function TrackerPage() {
  ...
  const filtered = companies
    ...
    .sort((a, b) => {
      if (sortMode === 'jobs')  return (b.jobs?.length ?? 0) - (a.jobs?.length ?? 0)
      if (sortMode === 'alpha') return a.name.localeCompare(b.name)
      return (a.tier - b.tier) || (a.sort_order - b.sort_order)
    })

  const totalJobs      = companies.reduce((n, c) => n + c.jobs.length, 0)
  const submittedCount = companies.reduce((n, c) => n + c.jobs.filter(j => j.status === 'submitted').length, 0)
  const interestedCount= companies.reduce((n, c) => n + c.jobs.filter(j => j.status === 'interested').length, 0)
  ...
  async function addJob(companyId: number) {
    ...
    setNewJobUrl(''); setNewJobTitle(''); setNewJobSalary('')
    ...
    <div className="min-h-screen bg-slate-50">
        ...
        <div className="max-w-5xl mx-auto px-4 py-3">
          <div className="flex items-center gap-3 mb-3 flex-wrap">
            ...
            <div className="flex gap-2 flex-wrap">
              <span className="text-xs bg-slate-100 text-slate-500 px-2 py-1 rounded-full font-mono">{companies.length} cos</span>
              <span className="text-xs bg-slate-100 text-slate-500 px-2 py-1 rounded-full font-mono">{totalJobs} jobs</span>
              <span className="text-xs bg-emerald-100 text-emerald-700 px-2 py-1 rounded-full font-mono">{submittedCount} submitted</span>
              <span className="text-xs bg-violet-100 text-violet-700 px-2 py-1 rounded-full font-mono">{interestedCount} interested</span>
            </div>
          </div>
          <div className="flex gap-2 flex-wrap">
            <Input
            ...
            <Select value={String(tierFilter)} onValueChange={v => setTierFilter(v === 'all' ? 'all' : Number(v))}>
      ...
      <div className="max-w-5xl mx-auto px-4 py-4 space-y-2">
        {filtered.map((company, i) => {
          ...
          return (
            <div key={company.id} className="bg-white border border-slate-200 rounded-lg shadow-sm overflow-hidden">
              {/* Company row */}
              <div
              ...
              >
                ...
                <span className={`w-2 h-2 rounded-full shrink-0 ${TIER_COLORS[company.tier]}`} title={TIER_LABELS[company.tier]} />
                ...
                <div className="flex-1 min-w-0 flex items-center gap-2 flex-wrap">
                  ...
                  {company.tag && (
                    <span className="text-xs text-slate-400 hidden sm:block truncate max-w-xs">{company.tag}</span>
                  )}
                </div>
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-xs text-slate-400 border border-slate-200 rounded px-2 py-1 hover:border-blue-400 hover:text-blue-600 transition-colors shrink-0 font-mono hidden sm:block"
                  onClick={e => e.stopPropagation()}
                >
                  careers →
                </a>
                ...
                <span className={`text-slate-300 text-xs transition-transform shrink-0 ${isExpanded ? 'rotate-180' : ''}`}>▼</span>
              </div>
                ...
                <div className="border-t border-slate-100 bg-slate-50/50">
                  {company.jobs.length > 0 && (
                    <div className="px-4 py-2 space-y-1">
                      {company.jobs.map(job => (
                        <div key={job.id} className="flex items-center gap-2 py-1.5 group">
                          <div className="flex-1 min-w-0">
                            {job.url ? (
                              ...
                              <span className="text-sm text-slate-700 block truncate">{job.title}</span>
                            )}
                            {job.salary_range && (
                              <span className="text-xs text-slate-400 font-mono">{job.salary_range}</span>
                            )}
                          </div>
                          {job.date_applied && (
                  ...
                  {isAddingJob && (
                    <div className="px-4 py-3 border-t border-slate-100 flex gap-2 flex-wrap items-center">
                      <Input className="h-8 text-sm flex-1 min-w-36" placeholder="Job title" value={newJobTitle} onChange={e => setNewJobTitle(e.target.value)} autoFocus />
                      <Input className="h-8 text-sm flex-1 min-w-48" placeholder="Job URL" value={newJobUrl} onChange={e => setNewJobUrl(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') addJob(company.id) }} />
                      <Input className="h-8 text-sm w-36" placeholder="Salary range" value={newJobSalary} onChange={e => setNewJobSalary(e.target.value)} />
                      <Button size="sm" className="h-8 text-xs" onClick={() => addJob(company.id)}>Add</Button>
                      <Button size="sm" variant="ghost" className="h-8 text-xs" onClick={() => { setAddingJobFor(null); setNewJobUrl(''); setNewJobTitle(''); setNewJobSalary('') }}>Cancel</Button>
                    </div>
                  )}
                  ...
                  {company.jobs.length === 0 && !isAddingJob && (
                    <div className="px-4 py-3 text-xs text-slate-400">
                      No jobs yet.{' '}
                      <button className="text-blue-500 hover:underline" onClick={() => setAddingJobFor(company.id)}>Add one</button>
                    </div>
                  )}
                </div>
              )}
