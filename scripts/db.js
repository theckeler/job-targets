  ...
  try {
    switch (command) {
      case "status": {
        const {
          rows: [c],
        } = await client.query("SELECT COUNT(*) FROM companies");
      ...
      case "all-jobs": {
        ...
        rows.forEach((r) => {
          console.log(
            `[${r.id}] ${r.company} — ${r.title || "Untitled"} (${r.status})`,
          );
          if (r.url) console.log(`     URL: ${r.url}`);
          if (r.salary_range) console.log(`     Salary: ${r.salary_range}`);
      ...
      case "add-job": {
        ...
        if (!companies.length)
          throw new Error(`Company not found: ${args.company}`);
        ...
        console.log(
          `Added job [${job.id}]: ${args.company} — ${job.title || "Untitled"}`,
        );
        break;
      }
      ...
      case "add-company": {
        ...
        console.log(
          `Added company [${company.id}]: ${company.name} (tier ${company.tier})`,
        );
        break;
      }
      ...
      default:
        console.log(
          "Commands: status | new-jobs | all-jobs | add-job | update-job | add-company",
        );
    }
  } finally {
    await client.end();
